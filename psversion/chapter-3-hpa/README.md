...
Write-Host "📊 Watching HPA (press Ctrl+C to stop):"
...
Write-Host "✅ Load generator started"
Write-Host "⏳ This will generate continuous load to trigger HPA scaling..."
```
...
Write-Host "📊 Monitoring HPA (checking every 15 seconds for 5 minutes):"
...
for ($i = 1; $i -le 20; $i++) {
  ...
  Write-Host "`nPod Count:"
  (kubectl get pods -n hpa-demo -l app=php-apache --no-headers | Measure-Object -Line).Lines
  
  Write-Host "`nPod Resource Usage:"
  $podMetrics = kubectl top pods -n hpa-demo -l app=php-apache --no-headers 2>$null
  if ($podMetrics) {
    $podMetrics
  } else {
...
```powershell
# Check HPA events
Write-Host "📋 HPA Scaling Events:"
kubectl describe hpa php-apache-hpa -n hpa-demo | Select-String -Pattern "Events:" -Context 0,20

# Check deployment events
Write-Host "`n📋 Deployment Events:"
kubectl get events -n hpa-demo --sort-by='.lastTimestamp' | Select-String -Pattern "scaled|hpa" -CaseSensitive:$false
...
Write-Host "⏳ Monitoring scale-down behavior (checking every 20 seconds for 5 minutes):"

for ($i = 1; $i -le 15; $i++) {
  Write-Host "=== Check $i/15 ($(Get-Date -Format 'HH:mm:ss')) ==="
  
  kubectl get hpa php-apache-hpa -n hpa-demo
  Write-Host "Pod count: $((kubectl get pods -n hpa-demo -l app=php-apache --no-headers | Measure-Object -Line).Lines)"
...
Write-Host "✅ Memory app deployed"
...
Write-Host "✅ Multi-metric HPA created"
```
...
Write-Host "`n📊 Current Resource Usage:"
...
Write-Host "✅ Controlled scaling HPA created"
```
...
Write-Host "`n📋 HPA Details:"
$hpas = @((kubectl get hpa -n hpa-demo -o jsonpath='{.items[*].metadata.name}').Trim() -split '\s+')
foreach ($hpa in $hpas) {
  Write-Host "=== $hpa ==="
...
Write-Host "✅ Moderate load generator started"
Write-Host "⏳ Monitoring for 3 minutes:"
...
Write-Host "`n📊 Deployment Replicas:"
...
Write-Host "`n📋 Recent Scaling Events:"
kubectl get events -n hpa-demo --sort-by='.lastTimestamp' | Select-String -Pattern "scaled" -CaseSensitive:$false
```
